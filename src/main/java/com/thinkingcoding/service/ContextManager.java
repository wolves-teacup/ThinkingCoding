...
public class ContextManager {
    ...
    public ChatMessage buildProjectContextMessage() {
        try {
            ...
            context.append("2. 使用原生工具调用 `write_file(path, content)`\n");
            ...
            context.append("- 查看文件内容：`read_file(path)`\n");
            context.append("- 列出目录：`list_directory(path)`\n");
            context.append("- 执行命令：`bash(command)`\n\n");

            context.append("**正确示例（用户：写一个HelloWorld）：**\n");
            context.append("好的，我来帮你创建一个简单的Java程序。\n\n");
            context.append("（随后通过 `write_file` 工具调用传入文件路径和完整代码内容）\n\n");

            context.append("**错误示例（不要这样做）：**\n");
            context.append("❌ 不要先调用 `bash(ls -la *.java)` 检查文件\n");
            context.append("❌ 不要先调用 `read_file(HelloWorld.java)` 检查文件\n");
            ...
            context.append("1. 使用 `read_file(path)` 读取现有内容\n");
            context.append("2. 等待系统返回文件内容\n");
            context.append("3. 根据用户要求修改代码\n");
            context.append("4. 使用 `write_file(path, content)` 写入新内容\n\n");

            context.append("## 🔧 工具调用规范（重要！你必须严格遵守！）\n\n");
            context.append("你可以调用以下工具来执行操作。**必须使用模型的原生 Tool Calling，不要在文本里拼接命令字符串。**\n\n");

            context.append("可用工具：\n");
            context.append("- 读取文件：`read_file(path)`\n");
            context.append("- 创建/覆盖文件：`write_file(path, content)`\n");
            context.append("- 执行命令：`bash(command)`\n");
            context.append("- 列出目录：`list_directory(path)`\n\n");
            ...
            context.append("- \"查看桌面有哪些文件\" → `list_directory(~/Desktop)` 或 `bash(ls -la ~/Desktop)`\n");
            context.append("- \"查看当前目录\" → `list_directory(.)` 或 `bash(ls -la)`\n");
            context.append("- \"查看sessions目录\" → `list_directory(sessions)` 或 `bash(ls -la sessions)`\n");
            context.append("- \"读取桌面的test.txt\" → `read_file(~/Desktop/test.txt)`\n");
            ...
            context.append("✅ 正确示例1：发起 `list_directory(sessions)` 调用后等待返回。\n\n");

            context.append("❌ 错误示例2：调用删除命令后，立刻在文本中宣称删除成功。\n");
            context.append("✅ 正确示例2：发起 `bash(rm sessions/*)` 调用后等待返回。\n\n");
            ...
            context.append("❌ 不要写：- 创建文件：write_file(path, content)  ← 这会被误认为要执行工具\n");
            context.append("❌ 不要写：- 读取文件：read_file(path)  ← 这会触发工具调用\n\n");
